...
export default function ProviderProfile() {
  ...
  async function fetchProfile() {
    try {
      ...
      const isProfileFilled =
        data.contact_no || data.description || data.address;
  ...
  async function updateProfile() {
    ...
    const res = await axiosClient.put("/providers/updateprofile", formData, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
...
>
  {/* ✅ Breadcrumbs moved inside card, above logo */}
      ...
      >
          ...
          <div className="text-center md:text-left flex-1">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 sm:gap-4 mt-2 sm:mt-4">
              <span className="text-xs sm:text-sm bg-gray-100 dark:bg-gray-700 px-2 py-1 sm:px-3 sm:py-1 rounded-full">
                establishment:{" "}
                <span className="font-semibold">{formData.establishment}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        {showAllFields && (
          <>
            ...
            >
              {["contact_no", "website_link", "address", "country"].map(
                (key, index) => {
                  const label =
